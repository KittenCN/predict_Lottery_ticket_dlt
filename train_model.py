...
def create_train_data(name, windows, ball_num=5):
    """ 创建训练数据
    :param name: 红/蓝 球
    :param windows: 训练窗口
    :param ball_num: 多少颗球
    ...
    if name == BOLL_NAME[0][0]:
        data = DATA[["{}号码_{}".format(name, num + 1) for num in range(5)]].values
    else:
        data = DATA[["{}号码_{}".format(name, num + 1) for num in range(2)]].values     
    ...
    if b_name == BOLL_NAME[0][0]:
        ...
        with tf.compat.v1.Session() as sess:
            ...
            for epoch in range(blue_epochs):
                ...
                saver = tf.compat.v1.train.Saver()
                saver.save(sess, "{}{}.{}".format(blue_ball_model_path, blue_ball_model_name, extension))
            pred_key[b_name] = blue_ball_model.pred_sequence.name
            if not os.path.exists(blue_ball_model_path):
                os.mkdir(blue_ball_model_path)
            saver = tf.compat.v1.train.Saver()
            saver.save(sess, "{}{}.{}".format(blue_ball_model_path, blue_ball_model_name, extension))
