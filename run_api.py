...
def get_predict_result():
    ...
    blue_name_list = [(BOLL_NAME[1], i + 1) for i in range(blue_sequence_len)]
    ...
    blue_data = data[["{}号码_{}".format(name[0], i) for name, i in blue_name_list]].values.astype(int) - 1
    if len(data) != 3:
    ...
    with blue_graph.as_default():
        ...
        blue_pred = blue_sess.run(softmax, feed_dict={
            "blue_inputs:0": blue_data.reshape(batch_size, windows_size, blue_sequence_len),
            "sequence_length:0": np.array([blue_sequence_len] * 1)
        })
    # 拼接结果
    ball_name_list = ["{}_{}".format(name[1], i) for name, i in red_name_list] + ["{}_{}".format(name[1], i) for name, i in blue_name_list]
